...
SELECT cities.name, cities.population, countries.id, cities.country_id 
FROM cities
...
SELECT * FROM countries WHERE government_form = 'Constitutional Monarchy' 
AND capital > 200 
...
-- have the population greater than 500, 000? The query should return the Country Name, City Name, District and Population.
...
SELECT countries.region, COUNT(countries.name) AS countries_count
FROM countries
...
SELECT countries.name, COUNT(cities.name) AS cities_count
FROM cities
JOIN countries ON cities.country_id = countries.id
